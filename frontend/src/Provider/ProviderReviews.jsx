...
//         `http://localhost:5000/api/reviews/provider/${storedUser.id}`
//       );
...
//     await axios.put(
//       `http://localhost:5000/api/reviews/${id}/report`
//     );
...
const ProviderReviews = () => {
  ...
  const fetchReviews = async () => {
    try {
      if (!userId) return;
      const res = await axios.get(
        `http://localhost:5000/api/reviews/provider/${userId}`
      );
  ...
  const handleReport = async (id) => {
    try {
      await axios.put(`http://localhost:5000/api/reviews/${id}/report`);
